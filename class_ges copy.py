...
# Función para clasificar basado en palabras clave con expresiones regulares
def clasificar_por_palabras_clave(diagnostico, diccionario):
    for categoria, valores in diccionario.items():
        for keyword in valores['PK']:
            if re.search(keyword, diagnostico, re.IGNORECASE):
                return categoria
    return "Sin Clasificar"

def evaluar_criterio_etario(row, diccionario, categoria):
    edad = row['EDAD']

    if categoria != "Sin Clasificar":
        edad_min = diccionario[categoria]['emin']
        edad_max = diccionario[categoria]['emax']

        # Asegurar que la edad es válida
        if isinstance(edad, (int, float)) and edad_min <= edad <= edad_max:
            return categoria
        else:
            return "Fuera de criterio etario"

    return "Sin Clasificar"


def evaluar_diagnosticos(row, diccionario):
    resultados = []

    for i in range(1, 4):
        columna = f"DIAGNOSTICO {i}"
        if pd.notna(row[columna]):
            preliminar = clasificar_por_palabras_clave(row[columna], diccionario)
            final = evaluar_criterio_etario(row, diccionario, preliminar)
            resultados.append(final)

    for resultado in resultados:
        if resultado not in ["Sin Clasificar", "Fuera de criterio etario"]:
            print(resultado)
            return resultado
    return "Sin Clasificar"

# ✅ Función corregida
def cargar_archivo_class_ges(df, diccionario):
        ...
        # Evaluar diagnósticos y asignar clasificación final
        df['CAT_GES'] = df.apply(lambda row: evaluar_diagnosticos(row, diccionario), axis=1)
